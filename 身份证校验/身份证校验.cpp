...
int jisuannianling()
{
	year = shenfenzhenghao.substr(6, 4);
	int yeartemp = 0;
	for (int i = 1; i <= 4; i++)
	{
		int temp = year[4 - i];
...
int main()
{
	while (1)
	{
		char querenshenfenzhenghao;
	shuru:
		cout << "请输入身份证号：";
		cin >> shenfenzhenghao;
		system("cls");
	panduan:
		cout << "请确认您输入的身份证号为：" << shenfenzhenghao << endl << "确认请按“Y”" << endl << "重新请按“N”" << endl;
		cin >> querenshenfenzhenghao;
		if (querenshenfenzhenghao == 'N' or querenshenfenzhenghao == 'n')
		{
			system("cls");
			goto shuru;
		}
		...
		{
			if (querenshenfenzhenghao != 'Y' and querenshenfenzhenghao != 'y')
			{
		...
		system("cls");
		int length = shenfenzhenghao.length();
		if (length != 18)
		{
		...
		int jiaoyanma = 0;
		jiaoyanma = shenfenzhenghaojiaoyan();
		if (jiaoyanma == shenfenzhenghao[17] - 48 or jiaoyanma == shenfenzhenghao[17] - 78 or jiaoyanma == shenfenzhenghao[17] - 110)
		{
			int nianling = jisuannianling();
			if (nianling >= 0 and nianling <= 150)
			{
				year = shenfenzhenghao.substr(6, 4);
				string month;
				string day;
				month = shenfenzhenghao.substr(10, 2);
				day = shenfenzhenghao.substr(12, 2);
				if (month >= "01" and month <= "12")
				{
					...
					{
						string diqu;
						diqu = shenfenzhenghao.substr(0, 6);
						diqutemp = 0;
						for (int i = 1; i <= 6; i++)
						{
						...
						if (a != "错误")
						{
							cout << "身份证号：" << shenfenzhenghao << endl;
							cout << "输入的校验码：" << shenfenzhenghao[17] << endl << "算出的校验码：" << jiaoyanma << endl;
							cout << "该身份证号有效。" << endl;
						}
						else
						{
		...
		string xingbie;
		xingbie = shenfenzhenghao.substr(16, 1);
		int xingbietemp = 0;
		xingbietemp = xingbie[0];
		cout << "性别：";
		if ((xingbietemp - 48) % 2 == 0)
		{
			cout << "女性" << endl;
		}
		else
